...
public class JsonUtils {
    ...
    static {
        ...
        //设置日期格式
        objectMapper.setDateFormat(new SimpleDateFormat(DateUtils.FORMAT_DEFAULT));
    ...
    public static void main(String[] args) {
      ...
      // String json= obj2String(user);

        String json= obj2StringPretty(user);
        //System.out.println(json);

       User user1= string2Obj(json,User.class);
       // System.out.println(user1);

        List<User> userList= Lists.newArrayList();
        userList.add(user);
        String userlistjson=obj2String(userList);
        //System.out.println(userlistjson);

        //json数组转出集合对象
        List<User> userList1=string2Obj(userlistjson, new TypeReference<List<User>>() {
        });
